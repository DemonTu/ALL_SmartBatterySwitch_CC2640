...
* Input        : 电池图标状态, 显示列(4列 0，16，32，48)，显示行(128/16=8行 0,16,32...)
* Output       : None
* return       : None
**************************************************************************/
void OLED_showBatteryBmp(uint8_t row, uint8_t line, uint8_t batteryState)
{
	uint8_t i,j;
	uint8_t x;
	uint8_t y;
	...
	for(x=0; x<16; x++){
		...
		j %= 16;
		for(i=0; i<8; i++){
			OLED_DrawPoint(line+i, y, temp1&0x01);
			OLED_DrawPoint((line+i+8), y, temp2&0x01);
			temp1>>=1;
			temp2>>=1;
		}
		y++;

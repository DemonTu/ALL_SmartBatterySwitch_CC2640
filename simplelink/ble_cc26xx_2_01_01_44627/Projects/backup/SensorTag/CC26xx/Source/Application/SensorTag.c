...
static const uint8_t devInfoHardwareRev[] = "V1.000";
...
static void SensorTag_taskFxn(UArg a0, UArg a1)
{
	...
	OLED_ShowString(0,48, "Gengee Test");  

	OLED_showBatteryBmp(0, 88, 1);
	OLED_ShowString(104,0, "90%");
		...
		if (!!(events & ST_PERIODIC_EVT))
		{
			...
			if (gapProfileState == GAPROLE_ADVERTISING)
			{
				SensorTag_blinkLed(Board_LED,1);
				{
					...
					SMB_Read(DESIGN_VOLTAGE, smbBuf, 2);
					bspUartWrite(smbBuf, 2);
					SMB_Read(VOLTAGE, smbBuf, 2);
					bspUartWrite(smbBuf, 2);
				}
				
				if (0 != testBuf[0])
				{
					bspUartWrite(testBuf, 4);
					memset(testBuf, 0, 4);
				}
			}
		}

#ifdef FEATURE_OAD  // 有定义
